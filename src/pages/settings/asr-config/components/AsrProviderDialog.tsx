...
import { AsrProviderConfig, CreateAsrProviderConfig } from '@/api/asr-config';

interface AsrProviderDialogProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  mode: 'create' | 'edit';
  editingProvider?: AsrProviderConfig;
  onSave: (data: CreateAsrProviderConfig) => Promise<void>;
  existingProviders: AsrProviderConfig[];
}
...
export const ASR_PROVIDER_OPTIONS: ProviderOption[] = [
  {
    code: 'doubao',
    name: '豆包语音',
...
const AsrProviderDialog: React.FC<AsrProviderDialogProps> = ({
  open,
  onOpenChange,
  mode,
  editingProvider,
  ...
  const [formData, setFormData] = useState<CreateAsrProviderConfig>({
    providerName: '',
    appId: '',
    accessToken: '',
    clusterId: 'volc.bigasr.sauc.duration',
    apiKey: '',
    ...
    if (!open) {
      ...
      if (mode === 'edit' && editingProvider) {
        ...
        setFormData({
          ...
          clusterId: editingProvider.clusterId || 'volc.bigasr.sauc.duration',
          apiKey: editingProvider.apiKey || '',
          modelName: editingProvider.modelName || 'gemini-2.5-flash',
  ...
  const handleFieldChange = (field: keyof CreateAsrProviderConfig, value: string) => {
  ...
  const handleSubmit = async () => {
    // Basic validation
    if (formData.providerName === 'doubao') {
  ...
  const renderStep1 = () => {
    ...
    const availableProviders = ASR_PROVIDER_OPTIONS.filter(
      ...
      return (
        <div className="space-y-4 py-4">
          <div className="text-center">
            ...
            <p className="mt-1 text-xs text-slate-600">请选择您要配置的语音识别服务商</p>
          </div>
        ...
        <div className="space-y-3">
          ...
          <div className="space-y-1.5">
            ...
            <Select
              ...
              onValueChange={(val) => handleFieldChange('clusterId', val)}
            >
              <SelectTrigger className="text-sm">
                <SelectValue placeholder="选择资源 ID" />
              </SelectTrigger>
              <SelectContent>
                <SelectItem value="volc.bigasr.sauc.duration">volc.bigasr.sauc.duration (按时长计费版)</SelectItem>
                <SelectItem value="volc.bigasr.sauc.concurrent">volc.bigasr.sauc.concurrent (按并发计费版)</SelectItem>
              </SelectContent>
            </Select>
        ...
        <div className="space-y-3">
          ...
          <div className="space-y-1.5">
            ...
            <Input
              ...
              placeholder="推荐使用: gemini-2.0-flash"
              className="text-sm"
            />
          </div>
          <div className="space-y-1.5">
...
export default AsrProviderDialog;
